...
-  **[Aura](https://github.com/BraianTroncoso/aura)** `private` — Your personal AI agent, like *Her*, living in Telegram. It reads your email, watches your calendar, tracks your assigned PRs and issues, and writes your daily standup — reasoning on a fully **local** model so your data never leaves your machine (no API key, no cost). Under the hood it runs the same agent runtime + `kb_*` knowledge base that scales to a multi-tenant product, but on minimal infra (SQLite + Ollama, no Docker, no Postgres/Neo4j/Redis). Not a chatbot. A second mind that remembers your context and acts on it. *(FastAPI + SQLAlchemy + SQLite + Ollama local LLM + Telegram + OpenClaw runtime)*
